...
if __name__ == "__main__":
    ...
    while True:
        mode = input("Play with a friend (1) or vs computer (2)? ")
        game = Game()
        if mode == "1":
            name1 = input("Enter Player 1 name: ")
            name2 = input("Enter Player 2 name: ")
            game.set_player1(HumanPlayer(name1, "X"))
            game.set_player2(HumanPlayer(name2, "O"))
        else:
            name1 = input("Enter your name: ")
            game.set_player1(HumanPlayer(name1, "X"))
            game.set_player2(ComputerPlayer("Computer", "O"))
        game.play()
        another_round = input("Do you want play again (y , n)?: ")
        if another_round == "y":
            continue
        break